...
- `deploy --template docker` generates a Docker build bundle under `.cyberfabric/<name>/deploy/` using deploy assets
...
- existing bundle directories under `.cyberfabric/` are replaced automatically, while existing custom
