...
const RUNTIME_CONFIG_PATH: &str = "/srv/config.yml";
...
impl DeployArgs {
    pub fn run(&self) -> anyhow::Result<()> {
        ...
        let config = common::get_config(&workspace_root, &config_path)?;
        ...
        common::generate_server_structure_at(
            &output_dir,
            &workspace_root,
            &project_name,
            Path::new(RUNTIME_CONFIG_PATH),
...
"#,
        ...
        assert!(generated_main.contains("/srv/config.yml"));
        assert!(generated_main.contains("config.opentelemetry.tracing.enabled"));
        assert!(generated_main.contains("init_tracing(&config.opentelemetry)"));
        assert!(generated_main.contains("otel_connectivity_probe(&config.opentelemetry)"));
